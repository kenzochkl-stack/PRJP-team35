...
export default function SelectSubjects() {
  ...
  const [selectionsByCycle, setSelectionsByCycle] = useState<Record<string, string[]>>({});

  const isAllCyclesFilled = cyclesArray.every(cycle => 
    selectionsByCycle[cycle] && selectionsByCycle[cycle].length > 0
  );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.tabsContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.centerScroll}>
          {cyclesArray.map((cycle) => (
            <TouchableOpacity 
              key={cycle} 
              onPress={() => { setActiveTab(cycle); setActiveFilter(''); setActiveSpe(''); }} 
              style={styles.tabItem}
            >
              <Text style={[styles.tabText, activeTab === cycle && styles.activeTabText]}>
                {cycle === "Secondaire" ? "Lycée" : (cycle === "Moyen" ? "Collège" : cycle)}
              </Text>
              {activeTab === cycle && <View style={[styles.underline, { backgroundColor: COLORS.primary }]} />}
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
      ...
      {(activeTab === "Secondaire" || activeTab === "ESI") && (
        <View style={styles.subFilterContainer}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.centerScroll}>
            {(activeTab === "Secondaire" ? ["TECHNIQUE", "LETTRE", "GESTION"] : ["1CP", "2CP", "1CS", "2CS"]).map((f) => (
              <TouchableOpacity key={f} onPress={() => {setActiveFilter(f); setActiveSpe('');}} style={styles.subFilterItem}>
                <Text style={[styles.subFilterText, activeFilter === f && { color: COLORS.primary }]}>{f}</Text>
                {activeFilter === f && <View style={[styles.subUnderline, { backgroundColor: COLORS.primary }]} />}
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
        ...
        <View style={styles.speRow}>
          {["SIQ", "SIL", "SID", "SIT"].map((s) => (
            <TouchableOpacity 
                ...
                style={[styles.speBtn, activeSpe === s && { backgroundColor: COLORS.primary, borderColor: COLORS.primary }]}
            >
              <Text style={[styles.speBtnText, activeSpe === s && { color: '#FFF' }]}>{s}</Text>
            </TouchableOpacity>
          ))}
        </View>
      )}
            ...
            return (
              <TouchableOpacity 
                ...
                style={[
                  ...
                  { backgroundColor: isSelected ? COLORS.secondary : '#FFF', borderColor: COLORS.primary }
                ]}
              >
      ...
      {isAllCyclesFilled && (
        <TouchableOpacity 
          ...
          onPress={() => {
            router.push({
              pathname: "/infoteacher", 
              params: { levels: levels, selectedSubjects: JSON.stringify(selectionsByCycle) }
            });
          }}
        >
...
const styles = StyleSheet.create({
  ...
  backBtn: { width: 45, height: 45, borderWidth: 1, borderColor: "#D8DADC", borderRadius: 10, justifyContent: "center", alignItems: "center", marginBottom: 30 },
  backIcon: { width: 18, height: 25 },
  mainTitle: { fontSize: scale(26), fontWeight: 'bold', textAlign: 'center', marginBottom: 30, color: '#000' },
  ...
  subFilterItem: { marginHorizontal: 12, paddingBottom: 5, alignItems: 'center' },
  subFilterText: { fontSize: 14, fontWeight: '600', color: '#999' },
  subUnderline: { height: 2, width: '100%', marginTop: 2 },
  subjectButton: { width: '100%', height: 50, borderWidth: 1.5, borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginBottom: 15 },
  subjectText: { fontSize: 16, fontWeight: '500', color: '#000' }, 
  speRow: { flexDirection: 'row', justifyContent: 'center', marginBottom: 15 },
  speBtn: { padding: 8, borderRadius: 8, borderWidth: 1, borderColor: '#EEE', marginHorizontal: 5 },
  speBtnText: { fontSize: 12, fontWeight: 'bold', color: '#666' },
  nextArrow: { position: 'absolute', bottom: 40, right: 30, width: 60, height: 60, backgroundColor: '#9F54F8', borderRadius: 12, justifyContent: 'center', alignItems: 'center', elevation: 5 },
